...
| 属性名 | 属性类型 | 功能说明 |
| ---- | ---- | ---- |
| WorkStatus | int | 面包机工作状态 |
| WFID | int | 菜单模式 |
| Color | int | 烧色 |
| Weight | int | 重量 |
| Button | int | 预约开关 |
| AppointmentTime | bytes | 预约时间 |
| WorkTime | int | 制作总时间 |
| KeepWarmTime | bytes | 保温总时间 |
| StepNum | int | 总步骤数 |
| CurrentStep | int | 当前步骤数 |
| MessageType | int | 提醒 |
| ErrorCode | int | 故障种类 |
| FuctionStatus | int | 功能状态 - 包含暂停启动状态|
| Identify | bytes | 云食谱id |
| SingleStepTime | bytes | 制作单步倒计时 |
| Tempreture | int | 实时温度（未启用） |
| WorkMode | bytes | 全步骤列表 |
| cmd1 | int |  |
| cmd2 | int |  |
